...
import { z } from "zod";
import { Persona, PersonaBlockKind, PersonaInjection } from "../src/persona/persona.js";
...
interface SourceSpan extends z.infer<typeof Fragment> {
  id: string;
  sourceId: string;
  characterId: string;
  sourceSha256: string;
...
/**
 * Eval-only, lossless source slicing. Inclusion policy remains owned by routePersona.
...
export function projectPersonaSources(input: readonly Persona[], rawProjection: unknown): {
  ...
  const projection = Projection.parse(rawProjection);
  ...
  for (const persona of personas) {
    persona.blocks = persona.blocks.flatMap((block) => {
      ...
      const fragments = plan.fragments.map((fragment) => {
        ...
        sourceIds.add(id);
        sourceSpans.push({
          ...fragment, id, sourceId: plan.blockId, characterId: persona.characterId,
